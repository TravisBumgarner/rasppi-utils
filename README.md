...
- **contest-scout** — monthly headless-Claude sweep for photo contests; updates the deadlines doc and pings Pushover.
...
| `contest-scout` | `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`) or `ANTHROPIC_API_KEY`; `PUSHOVER_APP_TOKEN` + `PUSHOVER_USER_KEY` (pushover.net); optional `CLAUDE_BIN`. Monthly systemd timer runs the repo's `/find-contests` skill headless, commits the updated `contest-deadlines.md`, and sends a Pushover nudge. Needs `npm install -g @anthropic-ai/claude-code` and git push auth on the Pi — see [contest-scout/README.md](contest-scout/README.md). |
